...
class Value:
    ...
    @classmethod
    # @classmethod 是一个Python的修饰器，它可以被用来修饰一个方法，使得该方法可以被类直接调用，而不需要创建类的实例。
    def make_const(cls, data, *, requires_grad=False):
        # 利用data创建一个不包含计算图的Value
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    # 这个node_to_output_grads_list是一个字典
    # key 是节点，value是一个列表，列表里面是当前节点对于每个输入的梯度
    # 这里就需要通过计算图来的反向AD传播来计算得到
    ...
    for node in reverse_topo_order:
        ...
        if node.is_leaf():
            continue
        # 利用运算符号的反向传播求梯度求解每个节点对于输入的梯度。
        v_input_2_node = node.op.gradient(node.grad, node)
        # 这里可能有由于返回的是一个Tensor或者是一个tuple， 所以需要转换成tuple方便后面的 zip迭代
        if isinstance(v_input_2_node, Tensor):
            v_input_2_node = [v_input_2_node]
        for k, grad in zip(node.inputs, v_input_2_node):
